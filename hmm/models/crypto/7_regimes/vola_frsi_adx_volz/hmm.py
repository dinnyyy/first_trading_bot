...
def time_series_cv(X, n_splits=5):
    ...
    for train_index, test_index in tscv.split(X):
        ...
        model = hmm.GaussianHMM(n_components=7, covariance_type="diag", n_iter=1000, random_state=42)
        model.fit(X_train)
...
state_names = [str(i) for i in range(26)]
...
print("\nTraining HMM model...")
model, scaler, X_scaled = train_hmm(train_data, features, n_components=26)
