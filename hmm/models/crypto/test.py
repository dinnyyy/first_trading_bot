...
def load_and_preprocess_data(file_path, years_of_data=10):
    ...
    # Scaled volatility
    df['Scaled_Vol'] = np.log1p(df['Volatility'])
...
# Main execution
features = ['Vol_Regime', 'Fisher_RSI', 'Trend_Strength', 'Volume_Zscore']

state_names = [str(i) for i in range(10)]

print("Starting main execution...")
file_path = r'C:\Users\joshd\OneDrive - Monash University\Projects\Python\Trading\Intro_using_AI\data\sp500_daily_5y.csv'
...
if not state_metrics_train_oos_df.empty:
    fig, axes = plt.subplots(2, 1, figsize=(14, 10), sharex=True)
    
    # Plot LogLikelihood
    state_metrics_train_oos_df.plot(x='Num_States', y='Train_LogLikelihood', kind='line', marker='o', ax=axes[0], label='Train LogLikelihood')
    state_metrics_train_oos_df.plot(x='Num_States', y='OOS_LogLikelihood', kind='line', marker='x', ax=axes[0], label='OOS LogLikelihood')
    axes[0].set_ylabel("LogLikelihood (Higher is better)")
    axes[0].set_title("HMM Model Selection: LogLikelihood (Train vs OOS)")
    axes[0].legend()
    axes[0].grid(True, linestyle='--', alpha=0.7)
    
    # Plot BIC
    state_metrics_train_oos_df.plot(x='Num_States', y='Train_BIC', kind='line', marker='o', ax=axes[1], label='Train BIC')
    state_metrics_train_oos_df.plot(x='Num_States', y='OOS_BIC', kind='line', marker='x', ax=axes[1], label='OOS BIC')
    axes[1].set_xlabel("Number of States")
    axes[1].set_ylabel("BIC (Lower is better)")
    axes[1].set_title("HMM Model Selection: BIC (Train vs OOS)")
    axes[1].legend()
    axes[1].grid(True, linestyle='--', alpha=0.7)
    
    plt.tight_layout()
    plt.savefig('hmm_state_selection_metrics_train_oos.png', dpi=300)
    plt.show()
    print("Train/OOS metrics plot saved as hmm_state_selection_metrics_train_oos.png")